...
{
    return m_appContext->glyphManager()->bitmapDimensionSize();
}

QVariant BitmapDimensionModel::data(const QModelIndex &index, int role) const
{
    if (!index.isValid() || index.row() >= m_appContext->glyphManager()->bitmapDimensionSize())
        return QVariant();

    QSharedPointer<BitmapDimension> dim = m_appContext->glyphManager()->bitmapDimensionAt(index.row());
