...
public:
    ...
    GlyphMeta(const QChar &newCharacter, int bitmapDimension, int glyphSize, const QFont &newFont = QFont(), const QString &newFontPath = QString(), bool temporary = false, bool dirty = true, bool resized = true)
        ...
        , m_offset(QPoint(0,0))
        , m_templateMarkup(QRect())
        , m_previewMarkup(QRect())
        , m_drawMarkup(QRect())
        , m_userRect(QRect())
        , m_dirty(dirty)
        , m_resized(resized)
    ...
    GlyphMeta(const GlyphMeta & glyphMeta)
        ...
        , m_templateMarkup(glyphMeta.m_templateMarkup)
        , m_previewMarkup(glyphMeta.m_previewMarkup)
        ...
        // , m_glyphMarkup(glyphMeta.m_glyphMarkup)
    {
    ...
    void setTemplateRect(const QRect &newGlyphRect) {
        m_templateMarkup = newGlyphRect;
    }

    QRect templateRect() {
        // return m_templateRect.translated(m_offset);
        return m_templateMarkup;
    }

    QSize templateSize(int factor = 1) {
        return m_templateMarkup.size() * factor;
    }

    void setPreviewRect(const QRect &newPreviewRect)
    {
        m_previewMarkup = newPreviewRect;
    }

    QRect previewRect ()
    {
        return m_previewMarkup;
    }
    ...
    int glyphRows() {
        return m_templateMarkup.height();
    }

    int glyphCols() {
        return m_templateMarkup.width();
    }

    int glyphLeft() {
        // return m_templateRect.translated(m_offset).left();
        return m_templateMarkup.left();
    }

    int glyphTop () {
        // return m_templateRect.translated(m_offset).top();
        return m_templateMarkup.top();
    }
    ...
    QString toString() {
        ...
        QString glyphRect = QString("(%1, %2, (%3 x %4))").arg(m_templateMarkup.left()).arg(m_templateMarkup.top()).arg(m_templateMarkup.width()).arg(m_templateMarkup.height());
...
// qDebug() << "operator <<";
        ...
        out << glyph.m_font << glyph.m_fontPath << glyph.m_character << glyph.m_glyphSize << glyph.m_bitmapDimension << glyph.m_templateMarkup << glyph.m_offset;
        return out;
    }

    friend QDataStream &operator>>(QDataStream &in, GlyphMeta &glyph) {
        in >> glyph.m_font >> glyph.m_fontPath >> glyph.m_character >> glyph.m_glyphSize >> glyph.m_bitmapDimension >> glyph.m_templateMarkup >> glyph.m_offset;
        return in;
    }
    ...
    QRect paintRect() {
        QRect glyphRect(m_templateMarkup);
...
private:
    ...
    GlyphMarkup m_templateMarkup; //< Пространство и положение (Left/Top), которое занимает глиф
    GlyphMarkup m_previewMarkup; //< Пространство и положение, которое занимает preview
    GlyphMarkup m_drawMarkup; //< Пространство пользовательского глифа
