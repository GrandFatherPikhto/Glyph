...
bool FontManager::loadFontContext(FontContext &context)
{
    if (context.font() == QFont())
        return false;

    m_fontContext = context;
...
bool FontManager::getRegisterFontFilePath()
{
    ...
    for (const QString &registerPath : std::as_const(m_fontRegisterPaths)) {
        ...
        if (!fontPath.isEmpty())
        {
            m_fontContext.setPath(fontPath);
...
QString FontManager::getFontPathOverRegisterKey(HKEY &hKey)
{
    ...
    while (true) {
        ...
        m_fontContext.font().family().toWCharArray(tchFontName);
